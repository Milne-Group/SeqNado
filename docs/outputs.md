...
#### Customising track groups

The `assay_config.ucsc_hub` settings control the hub layout. `supergroup_by`
creates the top-level collapsible groups, while `subgroup_by`, `overlay_by`, and
`color_by` control the tracks within them. In addition to built-in fields such
as `file_type`, `method`, `norm`, and `antibody`, every column in the configured
`metadata.csv` is available. For example, to create ChIP hub sections for each
condition × antibody combination:

```yaml
assay_config:
  ucsc_hub:
    supergroup_by: [file_type, condition, antibody]
    subgroup_by: [method, norm]
```

`condition` is matched to tracks via `sample_id`; user-defined metadata columns
work the same way. Values not applicable to a derived track (such as a
condition-level aggregate) are represented as `NA`.
