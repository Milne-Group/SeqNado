...
class TestSeqnadoConfigValidation:
    ...
    def test_csaw_scaling_defaults_to_bamnado_configured(self, tmp_metadata, tmp_bowtie_index):
        ...
        config = SeqnadoConfig(
            ...
            metadata=tmp_metadata,
            assay_config=ChIPAssayConfig(
                bigwigs=BigwigConfig(scale_methods=["unscaled", "csaw"])
            ),
        )

        assert config.third_party_tools.bamnado is not None

    def test_csaw_scaling_without_bamnado_raises(self, tmp_metadata, tmp_bowtie_index):
    ...
    def test_rna_csaw_scaling_filtered_out(self, tmp_metadata, tmp_star_index):
        ...
        assert config.assay_config.bigwigs.scale_methods == ["unscaled"]
